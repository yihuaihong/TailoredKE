...
def generate_fast(
    ...
    with torch.no_grad():
        while input_ids.size(1) < max_out_len:  # while not exceeding max output length
            ...
            """
            log_softmax_tensor = torch.log(softmax_out)  # 计算对数 softmax
            entropy = -torch.sum(softmax_out * log_softmax_tensor, dim=1)  # 在 dim=1 上进行求和
            ...
            for i in range(batch_size):
                ...
                if new_idx < max_out_len:
                    ...
                    new_tokens_list.append(new_toks[i]) #新添加
    ...
    new_tokens_list = list(chain.from_iterable([tok.convert_ids_to_tokens(tokens) for tokens in new_tokens_list]))

    print('new_tokens_list: ',new_tokens_list)

    """
    plt.figure(figsize=(20, 25))
    sns.heatmap([entropy_list], annot=True, cmap="YlGnBu", xticklabels=new_tokens_list)
    plt.show()
    """
